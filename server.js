...
const LOG_BLUEPRINT = {
  ...
  channels: {
    ...
    permissions: '🛡️・permissions-logs',
...
const LOG_ROLE_BLUEPRINT = {
  login: { name: '🔐 Login Logs', color: 0x8b5cf6 },
  announcement: { name: '📢 Announcement Logs', color: 0xec4899 },
  subscription: { name: '✅ Subscription Logs', color: 0x22c55e },
  permissions: { name: '🛡️ Permissions Logs', color: 0x3b82f6 },
  purchase: { name: '🧾 Purchase Logs', color: 0xf59e0b },
  invoice: { name: '💸 Invoice Logs', color: 0x10b981 },
  member: { name: '👥 Member Logs', color: 0x06b6d4 },
  message: { name: '💬 Message Logs', color: 0x6366f1 },
  channel: { name: '📁 Channel Logs', color: 0xa855f7 },
  role: { name: '🎭 Role Logs', color: 0xd946ef },
  voice: { name: '🔊 Voice Logs', color: 0x14b8a6 },
  moderation: { name: '🔨 Moderation Logs', color: 0xef4444 },
  server: { name: '🌐 Server Logs', color: 0x0ea5e9 },
  system: { name: '⚙️ System Logs', color: 0x64748b },
  error: { name: '❌ Error Logs', color: 0xdc2626 }
};
...
async function ensureLogInfrastructure(){
  ...
  const out = { guildId: guild.id, categoryId: category.id, channels: {}, roles: {} };

  for (const [key, roleDef] of Object.entries(LOG_ROLE_BLUEPRINT)) {
    let role = guild.roles.cache.find(r => r.name === roleDef.name) || null;
    if (!role) {
      role = await guild.roles.create({
        name: roleDef.name,
        color: roleDef.color,
        mentionable: false,
        reason: 'WM STORE auto log roles'
      });
    }
    out.roles[key] = role.id;
  }
...
async function sendLogToDiscord(logData){
  ...
  const roleId = state.roles ? (state.roles[bucket] || state.roles.system) : null;
  if (!channelId) return;
  ...
  try {
    ...
    const embed = new EmbedBuilder()
      .setColor('#7c3aed')
      .setTitle(`${emojiMap[bucket] || '📌'} WM STORE LOG | ${bucket.toUpperCase()}`)
      .addFields(
...
async function registerCommands(){
  ...
  const commands = [
    new SlashCommandBuilder().setName('setup-subscription-panel').setDescription('إرسال بانل الاشتراك في الإعلانات داخل هذه القناة').setDefaultMemberPermissions(PermissionsBitField.Flags.Administrator),
    new SlashCommandBuilder().setName('setup-invoice-panel').setDescription('إرسال بانل الفاتورة داخل هذه القناة').setDefaultMemberPermissions(PermissionsBitField.Flags.Administrator)
  ].map(c => c.toJSON());
...
function publicBaseUrl(req){
  return clean(process.env.PUBLIC_BASE_URL) || `${req.protocol}://${req.get('host')}`;
}
...
function invoiceTemplate(data){
  return {
    ...
    embeds: [
      new EmbedBuilder()
        .setColor('#7c3aed')
        .setTitle(`فاتورة شراء | ${data.storeName}`)
        .addFields(
          { name: 'المنتج', value: String(data.productName || '-').slice(0,1024), inline: true },
          { name: 'السعر', value: String(data.price || '-').slice(0,1024), inline: true },
          { name: 'المشتري', value: `<@${data.buyerUserId}>`, inline: true },
          { name: 'البائع', value: String(data.sellerName || '-').slice(0,1024), inline: true },
          { name: 'المشرف', value: String(data.supervisorName || '-').slice(0,1024), inline: true },
          { name: 'معلومات المتجر', value: String(data.storeInfo || '-').slice(0,1024), inline: false },
          { name: 'ملاحظات', value: String(data.notes || '-').slice(0,1024), inline: false },
          { name: 'Developer', value: String(data.developerInfo || '-').slice(0,1024), inline: false }
        )
        .setFooter({ text: 'WM STORE Invoice System' })
        .setTimestamp()
    ]
  };
}
...
app.get('/invoices', auth, ownerOrAdmin, async (req,res) => {
  res.render('invoices', {
    currentUser: me(req),
    settings: read(SETTINGS),
    developer: await developerProfile(),
    invoices: read(INVOICES).slice(0, 100),
    result: null
  });
});

app.post('/invoice/send', auth, ownerOrAdmin, async (req,res) => {
  const currentUser = me(req);
  try {
    const buyerUserId = clean(req.body.buyerUserId);
    const productName = clean(req.body.productName);
    const price = clean(req.body.price);
    const sellerName = clean(req.body.sellerName);
    const supervisorName = clean(req.body.supervisorName);
    const storeInfo = clean(req.body.storeInfo);
    const notes = clean(req.body.notes);

    if (!buyerUserId || !productName || !price || !sellerName || !supervisorName) {
      throw new Error('يرجى تعبئة كل الحقول الأساسية للفاتورة.');
    }

    const dev = await developerProfile();
    const invoice = {
      id: String(Date.now()),
      buyerUserId,
      productName,
      ...
      supervisorName,
      storeName: 'WM STORE',
      storeInfo: storeInfo || 'WM STORE | Digital Products & Services',
      notes: notes || '-',
      developerInfo: `${dev.displayName} | @${dev.username}`,
      createdBy: currentUser.email,
      createdAt: new Date().toISOString()
    };

    const buyer = await client.users.fetch(buyerUserId, { force: true });
    await buyer.send(invoiceTemplate(invoice));

    const inv = read(INVOICES);
    inv.unshift(invoice);
    write(INVOICES, inv.slice(0, 500));

    addLog({
      type: 'invoice_send',
      actorEmail: currentUser.email,
      message: `Invoice sent to ${buyerUserId} for ${productName} (${price})`
    });

    addLog({
      type: 'purchase_complete',
      actorEmail: currentUser.email,
      message: `Purchase completed | Product: ${productName} | Price: ${price} | Buyer: ${buyerUserId} | Seller: ${sellerName} | Supervisor: ${supervisorName}`
    });
    ...
    res.render('invoices', {
      ...
      developer: dev,
      invoices: read(INVOICES).slice(0, 100),
      result: { success: true, message: 'تم إرسال الفاتورة للمشتري بالخاص.' }
    });
  } catch (err) {
    res.render('invoices', {
      currentUser,
...
client.on('interactionCreate', async interaction => {
  try{
    ...
    if(interaction.isChatInputCommand() && interaction.commandName === 'setup-invoice-panel'){
      const url = clean(process.env.PUBLIC_BASE_URL) ? `${clean(process.env.PUBLIC_BASE_URL)}/invoices` : null;
      const embed = new EmbedBuilder()
        .setColor('#7c3aed')
        .setTitle('WM STORE | بانل الفاتورة')
        .setDescription('هذا البانل مخصص للبائعين فقط. من خلاله تقدر تعبّي بيانات المنتج والسعر والبائع والمشرف وترسل الفاتورة للمشتري بالخاص.')
        .setFooter({ text: 'WM STORE Seller Invoice Panel' })
        .setTimestamp();

      if (url) {
        const row = new ActionRowBuilder().addComponents(
          new ButtonBuilder().setLabel('فتح بانل الفاتورة').setStyle(ButtonStyle.Link).setURL(url)
        );
      ...
      addLog({ type:'invoice_panel_posted', actorEmail:'bot', message:`Invoice panel posted in channel ${interaction.channelId}` });
      return interaction.reply({ content:'تم إرسال بانل الفاتورة بنجاح.', ephemeral:true });
    }
